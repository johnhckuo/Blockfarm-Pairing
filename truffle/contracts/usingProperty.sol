...
contract usingProperty{
    ...
    function getProperty(uint i) constant returns(uint, uint, bytes32, uint, uint, uint){
        return ( propertyList[i].id, propertyList[i].propertyType, propertyTypeList[propertyList[i].propertyType].name, propertyList[i].propertyCount, propertyList[i].tradeable,  propertyList[i].owner);
    }
