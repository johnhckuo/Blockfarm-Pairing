...
if (Meteor.isClient) {
  ...
  Template.matchmaking.events({
      ...
      'click #test': function () {
          //CongressInstance.addMember('John', { from: web3.eth.accounts[0], gas: 2000000 }, function (err, res) {
          //    if (err) {
          //        console.log(err);
          //    }
          //    else {
          ...
          //    }
          //});
          usingPropertyInstance.addPropertyType('Car', { from: web3.eth.accounts[0], gas: 2000000 }, function (err, res) {
              if (err) {
                  console.log(err);
              }
              else {
          ...
          usingPropertyInstance.addPropertyType('Shirt', { from: web3.eth.accounts[0], gas: 2000000 }, function (err, res) {
              if (err) {
                  console.log(err);
              }
              else {
          ...
          usingPropertyInstance.addPropertyType('Real Estate', { from: web3.eth.accounts[0], gas: 2000000 }, function (err, res) {
              if (err) {
                  console.log(err);
              }
              else {
  ...
  Template.matchmaking.helpers({
      ...
      propertyTypes: function () {
          ...
          for (var i = 0 ; i < propertyType.length; i++) {
              data.push({
                  "propertyTypeId": propertyType[i].id,
                  "name": propertyType[i].name,
              });
          }     
          return data;
      }
